        ...
        <div class="row">
            ...
            if (!empty($this->data['items']) <= 0) {
                ...
                for ($i = 0; $i < $n; ++$i) {
                    ...
                    $link_edit = base_url('/admin_restaurants.php?action=edit&id='.$row['id'].'&per_page='.$per_page);
                    $link_restore = base_url('/admin_restaurants.php?action=restore&id='.$row['id'].'&per_page='.$per_page);
                    $link_trash = base_url('/admin_restaurants.php?action=trash&id='.$row['id'].'&per_page='.$per_page);
                    $link_delete = base_url('/admin_restaurants.php?action=delete&id='.$row['id'].'&per_page='.$per_page);

                    $status_link = base_url('/admin_restaurants.php?action=unpublish&id='.$row['id'].'&per_page='.$per_page);
                    if ($row['status'] == 0) {
                        $status_link = base_url('/admin_restaurants.php?action=publish&id='.$row['id'].'&per_page='.$per_page);
                    } ?>
                                <div class="row gy-2">
